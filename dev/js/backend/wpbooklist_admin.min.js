...
console.log( 'Thisssj is the JavaScript Object that holds all PHP Variables for use in JavaScript' );
...
jQuery( document ).ready( function( $ ) {
	...
	function wpbooklistDashboardChooseBookCoverImageButton() {
		...
		//if ( undefined !== wp && undefined !== wp.media && undefined !== wp.media.model && undefined !== wp.media.model.settings && undefined !== wp.media.model.settings.post && undefined !== wp.media.model.settings.id ) {

			// Store the old id
			//wpMediaPostId = wp.media.model.settings.post.id;

			// Must be parsed to an int! All the wpbooklistPhpVariables are strings initially.
			setToPostId = parseInt( wpbooklistPhpVariables.SAVED_ATTACHEMENT_ID );
			...
			$( document ).on( 'click', '.wpbooklist-addbook-upload_image_button, #wpbooklist-storefront-img-button-1, #wpbooklist-storefront-img-button-2, #wpbooklist-branding-img-button-1, #wpbooklist-branding-img-button-2, #wpbooklist-editbook-upload_image_button', function( event ) {
				...
				if ( fileFrame ) {
					/*
					This prevents uploading a new image twice 
	...
	function wpbooklistDashboardAddBook() {
		...
		$( '#wpbooklist-admin-addbook-create-button' ).on( 'click', function() {
			...
			var ebookurl = $( '#wpbooklist-addbook-ebook-file' ).val();
			var genrestring = '';
			var subgenrestring = '';
			var keywordstring = '';
			...
			if ( null !== upsells  && undefined !== upsells ) {
				for ( var i = 0; i < upsells.length; i++ ) {
					upsellString = upsellString + ', ' + upsells[i];
				};
			}

			if ( null !== crosssells && undefined !== crosssells ) {
				for ( var i = 0; i < crosssells.length; i++ ) {
					crosssellstring = crosssellstring + ', ' + crosssells[i];
				};
			}

			if ( null !== salebegin && undefined !== salebegin && null !== saleend && undefined !== saleend ) {
			...
			if ( null !== datefinished && undefined !== datefinished ) {
		...
		$( document ).on( 'click', '.wpbooklist-admin-editbook-edit-button', function( event ) {
			...
			if ( -1 < window.location.href.indexOf( '?page=WPBookList-Options-books&tab=edit-books' ) ) {
				...
				if ( false !== error ) {
					...
					// Make some checks for blank prices...
					if ( ( '' === price || null === price ) && ( '' !== regularPrice && null !== regularPrice ) ) {
						price = regularPrice;
					}
					if ( ( '' === regularPrice || null === regularPrice ) && ( '' !== price && null !== price ) ) {
						regularPrice = price;
					}
					if ( ( ( '' === regularPrice || null === regularPrice ) && ( '' === price || null === price )   )       && ( '' !== saleprice && null !== saleprice ) ) {
	...
	function wpbooklistToggleCheckAllLibraryDisplayOptions() {
		...
		$( '#wpbooklist-check-all' ).on( 'click', function() {
			...
			$( '.wpbooklist-display-options-indiv-entry input' ).each( function() {
		...
		$( document ).on( 'click', '.wpbooklist-edit-actions-edit-button', function( event ) {
			...
			request = $.ajax({
				...
				success: function( response ) {
					...
					// Make some mods to prevent '0' showing up in Pub and Orig Pub Year.
					if ( '0' === bookInfo.originalpubyear || 0 === bookInfo.originalpubyear ) {
						bookInfo.originalpubyear = '';
					}
					if ( '0' === bookInfo.pub_year || 0 === bookInfo.pub_year ) {
						bookInfo.pub_year = '';
					}
					...
					$( '.wpbooklist-addbook-customfield-textlink-text-input' ).each( function(){
						customdbname = $( this ).attr( 'name' );
						console.log('wtf')
						customdbname = customdbname.split( '-' );
						customdbname = customdbname[4];
						console.log(customdbname);
						console.log(bookInfo);
						if( null !== bookInfo[ customdbname ] ) {
							customdbname = bookInfo[ customdbname ].split( '---' );
							$( this ).val( customdbname[0] );
						}
					});

					$( '.wpbooklist-addbook-customfield-textlink-url-input' ).each( function(){
						customdbname = $( this ).attr( 'name' );
						customdbname = customdbname.split( '-' );
						customdbname = customdbname[4];
						if( null !== bookInfo[ customdbname ] ) {
					...
					$( '.wpbooklist-addbook-image-url-image-input' ).each( function(){
						customdbname = $( this ).attr( 'name' );
						customdbname = customdbname.split( '-' );
						customdbname = customdbname[4];
						customdbname = bookInfo[ customdbname ].split( '---' );
					...
					$( '.wpbooklist-addbook-image-url-href-input' ).each( function(){
						customdbname = $( this ).attr( 'name' );
						customdbname = customdbname.split( '-' );
						customdbname = customdbname[4];
						if( null !== bookInfo[ customdbname ] ) {
					...
					// Now populate any Plain Text Custom Fields.
					$( '.wpbooklist-addbook-customfield-paragraph-input' ).each( function(){
						customdbname = $( this ).attr( 'name' );
						customdbname = customdbname.split( '-' );
						customdbname = customdbname[2];
						$( this ).val( bookInfo[ customdbname ] );
					...
					// Now populate any Dropdown Custom Fields.
					$( '.wpbooklist-addbook-customfield-dropdown-entry' ).each( function(){
						customdbname = $( this ).attr( 'name' );
						customdbname = customdbname.split( '-' );
						customdbname = customdbname[2];

						if ( '' !== bookInfo[ customdbname ] && null !== bookInfo[ customdbname ] ) {
							$( this ).val( bookInfo[ customdbname ] );
						} else {
							$( this ).val( 'default' );
						}
					...
					decoded = $( '<textarea/>' ).html( bookInfo.shortdescription ).text();
					decoded2 = $( '<textarea/>' ).html( decoded ).text();
					decoded2 = decoded2.replace( /\\/g, '' );

					$( '#wpbooklist-addbook-shortdescription' ).val( decoded2 );

					if ( null !== bookInfo.rating && 0 !== bookInfo.rating && '0' !== bookInfo.rating ) {
						$( '#wpbooklist-addbook-select-book-rating' ).val( bookInfo.rating );
					} else {
						$( '#wpbooklist-addbook-select-book-rating' ).val( 'default' );
					}
					...
					// Now populate the E-Books Downloads, if applicable
					$( '#wpbooklist-addbook-ebook-file' ).val( bookInfo.ebook );
	...
	function wpbooklistEditBookPagination() {
		...
		$( document ).on( 'click', '#wpbooklist-edit-next-100, #wpbooklist-edit-previous-100', function( event ) {
			...
			var middlePagination = $( '.wpbooklist-pagination-middle-div-select' ).val();
			...
			var request = $.ajax({
				...
				success: function( response ) {
					...
					if ( 'back' === direction ) {
						...
						console.log('value is: ' + ( parseInt( middlePagination ) - wpbooklistPhpVariables.EDIT_PAGE_OFFSET ) );
						console.log('currentoffset is : ' + currentOffset );
						$( '.wpbooklist-pagination-middle-div-select' ).val( currentOffset );
					} else {
		...
		$( document ).on( 'change', '.wpbooklist-pagination-middle-div-select', function( event ) {
			...
			$( '.wpbooklist-edit-book-indiv-div-class' ).animate({ 'opacity': '0.3'}, 500 );
			$( '#wpbooklist-spinner-pagination' ).animate({ 'opacity': '1'}, 500 );
			initialNextOpacity = $( '#wpbooklist-edit-next-100' ).css( 'opacity' );
			initialPrevOpacity = $( '#wpbooklist-edit-previous-100' ).css( 'opacity' );

			$( '#wpbooklist-edit-next-100' ).css({ 'opacity': '0.3' });
			$( '#wpbooklist-edit-previous-100' ).css({ 'opacity': '0.3' });

			// Grabbing library
			library =  $( '#wpbooklist-editbook-select-library' ).val();
			...
			request = $.ajax({
				...
				success: function( response ) {
					...
					if ( 1 === initialNextOpacity ) {
						$( '#wpbooklist-edit-next-100' ).css({ 'opacity': '1' });
					}

					if ( 1 === initialPrevOpacity ) {
						$( '#wpbooklist-edit-Previous-100' ).css({ 'opacity': '1' });
					}
					...
					// Grabbing offset from dom
					currentOffset = $( '.wpbooklist-admin-top-title' ).attr( 'data-offset' );
					dataLimit = $( '.wpbooklist-pagination-middle-div-select' ).attr( 'data-limit' );

					console.log( 'currentOffset is: ' + currentOffset );
					console.log( 'dataLimit is: ' + dataLimit );

					console.log( 'currentoffset plus edit_page_offset is: ' + (parseInt( currentOffset ) + parseInt( wpbooklistPhpVariables.EDIT_PAGE_OFFSET ) ) );

					if ( parseInt( currentOffset ) + parseInt( wpbooklistPhpVariables.EDIT_PAGE_OFFSET ) >= parseInt( dataLimit ) ) {
					...
					if ( parseInt( wpbooklistPhpVariables.EDIT_PAGE_OFFSET ) >= parseInt( currentOffset ) ) {
	...
	function wpbooklistEditBookSwitchLib() {
		$( document ).on( 'change', '#wpbooklist-editbook-select-library', function( event ) {
			...
			if ( window.location.href.includes( 'library=' ) && window.location.href.includes( 'tab=' ) && window.location.href.includes( 'WPBookList' ) ) {
				newUrl = ( window.location.href.substr( 0, window.location.href.lastIndexOf( '&' ) ) ) + '&library=' +library;
			} else {
				newUrl = window.location.href + '&library=' + library;
			}
